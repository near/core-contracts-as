...
//returns promise
function loadWASM (wasmFile) {

    //fulfill imported external fns from the WASM
    const importObject = {
        env: require("./js-mocked-env")
    };
    //load wasm file
    let fileBuffer = fs.readFileSync(wasmFile)

    //instantiate as asm.js  code -- returns Promise
...
Account.prototype.call = function (contractAccount, method, input, attachedDeposit) {
    ...
    if (contractAccount.code == undefined) throw new Error("destination account " + contractAccount.accountId + " has no smart contract deployed")
    ...
    console.log("------ " + 
        this.accountId + " CALL" + 
        (attachedDeposit!=undefined? " with attachedDeposit:"+attachedDeposit: "") + " " +
        contractAccount.accountId + '.' + method + "(" + (input ? JSON.stringify(input) : "") + ")");
